...
def cropping(img: np.array) -> np.array:
    """
    ...
    transversal_thresh = 0.75  # 190/255
    ...
    cnt_crit_col = np.max(5,int(width*0.01))
    ...
    left_crop = np.argmax(merge_col_cond_left)
    ...
    bottom_crop = np.min(height-1, bottom_crop + (bottom_crop-top_crop) * 0.18)
